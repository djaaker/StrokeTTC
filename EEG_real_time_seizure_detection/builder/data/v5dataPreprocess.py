...
class Args:
    ...
    samples_per_class = 200  # Number of samples per class for balancing
    verbose = True  # Set to True to enable verbose logging

args = Args()

# Corrected collection of EDF files
healthyEdfs = []
seizEdfs = []

for subject_path in names:
    subject_name = os.path.basename(subject_path)
    subject_label = df[df['name'] == subject_name]['label'].values[0]
    ...
    if subject_label == 0 or subject_label == 3:
        ...
        # Print a sample of the edf files collected for this subject
        if args.verbose:
...
# Make samples_per_class adjustable via args
mlData = mlData.groupby('label').head(args.samples_per_class).reset_index(drop=True)
...
#Print the count of each label in labelFrame to see if there are any subjects with label 0.
print(labelFrame['label'].value_counts())
